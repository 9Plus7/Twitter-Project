...
public class MessageDAOclass implements MessageDAO {
    ...
    public List<Messages> searchUserMessages(String keyword) {
        ...
        Map<String,Object> parameters = new HashMap<String,Object>();
        parameters.put("UserID",user_id);
        String sqlquery = "SELECT * FROM messages WHERE (user_id=:UserID OR user_id IN (SELECT follower_user_id FROM followers WHERE user_id=:UserID)) AND content LIKE '%" + keyword + "%' ORDER BY id";
        List<Messages> result = myJDBCTemplate.query(sqlquery,parameters,myMessageMapper);
        return result;
    }
    ...
    public int getId() {
        ...
        String sqlquery = "SELECT id FROM user WHERE handle = :handle";
